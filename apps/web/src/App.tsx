...
export function App() {
  ...
  return <div className={`shell ${trace ? "show-trace" : ""} ${theme === "light" ? "light-theme" : ""}`}>
    <aside className="sidebar">
      ...
      <nav className="workspace-groups">{workspaces.loading && <small>正在读取工作空间…</small>}{!workspaces.loading && workspaces.workspaces.map((workspace) => { const chats = workspaceChats[workspace.name as keyof typeof workspaceChats] ?? []; const current = workspace.id === workspaces.currentWorkspaceId; return <section className={`workspace-group ${current ? "current" : ""}`} key={workspace.id}><header><button className="workspace-name" onClick={() => workspace.status === "available" && void workspaces.selectWorkspace(workspace.id)}><i>⌄</i><b>▰</b><span>{workspace.name}</span>{current && <em>当前</em>}<small>{chats.length}</small></button><button aria-label={`在 ${workspace.name} 新建对话`} className="workspace-add">＋</button></header><div className="workspace-conversations">{chats.map((chat, index) => <button className={current && index === 0 ? "active" : ""} key={chat}><span>{chat}</span><i>{index ? "昨天" : "刚刚"}</i></button>)}{chats.length === 0 && <p>暂无对话</p>}</div></section>; })}</nav>
