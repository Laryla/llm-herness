...
import { CheckCircle2, Clock3, LoaderCircle, Plus, RefreshCw, Server, XCircle } from "lucide-react";
...
export function ModelSettings({ state }: { state: ModelState }) {
  ...
  async function submitProfile(event: FormEvent<HTMLFormElement>) {
    ...
    const created = await state.createProfile(secretMode === "local"
  ...
  async function testConnection(profileId: string) {
    ...
    try { await state.testConnection(profileId); }
