...
export function ModelSettings({ state }: { state: ModelState }) {
  ...
  async function testConnection(profileId: string, modelName: string) {
    ...
    try {
      ...
      if (tested?.connection.status === "succeeded") setConnectionNotice(`连接成功，已使用 ${modelName} 完成测试`);
      else if (tested?.connection.status === "failed") setConnectionNotice(`连接失败：${tested.connection.error.message}`);
    }
